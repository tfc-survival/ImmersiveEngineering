...
public class BlockWoodenDevice1 extends BlockIETileProvider<BlockTypes_WoodenDevice1> implements IPostBlock {
    public BlockWoodenDevice1() {
        ...
        lightOpacity = 0;
        setMetaMobilityFlag(BlockTypes_WoodenDevice1.POST.getMeta(), EnumPushReaction.BLOCK);
        setMetaMobilityFlag(BlockTypes_WoodenDevice1.WATERMILL.getMeta(), EnumPushReaction.BLOCK);
        setMetaMobilityFlag(BlockTypes_WoodenDevice1.WINDMILL.getMeta(), EnumPushReaction.BLOCK);
    }

    @Override
    public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        if (getMetaFromState(state) == BlockTypes_WoodenDevice1.POST.getMeta())
            return;
    ...
    public void breakBlock(World world, BlockPos pos, IBlockState state) {
        ...
        if (tileEntity instanceof TileEntityWoodenPost) {
            if (!((TileEntityWoodenPost) tileEntity).isDummy() && !world.isRemote && world.getGameRules().getBoolean("doTileDrops") && !world.restoringBlockSnapshots)
                world.spawnEntity(new EntityItem(world, pos.getX() + .5, pos.getY() + .5, pos.getZ() + .5, new ItemStack(this, 1, getMetaFromState(state))));
        }
    ...
    public boolean canIEBlockBePlaced(World world, BlockPos pos, IBlockState newState, EnumFacing side, float hitX, float hitY, float hitZ, EntityPlayer player, ItemStack stack) {
        ...
        } else if (stack.getItemDamage() == BlockTypes_WoodenDevice1.WINDMILL.getMeta() || stack.getItemDamage() == BlockTypes_WoodenDevice1.WINDMILL_ADVANCED.getMeta()) {
            EnumFacing f = EnumFacing.fromAngle(player.rotationYaw);
            System.out.println(f);
            for (int ww = 0; ww <= 20; ww++) {
                BlockPos pos2 = pos.add(f.getAxis() == Axis.X ? ww : 0, 0, f.getAxis() == Axis.X ? 0 : ww);
                if (world.isOutsideBuildHeight(pos2) || !world.getBlockState(pos2).getBlock().isReplaceable(world, pos2))
                    return false;
            }
