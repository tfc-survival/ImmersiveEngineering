...
public class TileEntityCokeOven extends TileEntityMultiblockPart<TileEntityCokeOven> implements IIEInventory, IActiveState, IGuiTile, IProcessTile {
    ...
    public void update() {
        ...
        if (!world.isRemote && formed && !isDummy()) {
            ...
            if (process > 0) {
                ...
                if (active) {
                    ...
                    if (recipe != null) {
                        ...
                        tank.fill(new FluidStack(IEContent.fluidCreosote, recipe.creosoteOutput), true);
                    }
                    processMax = 0;
                    active = false;
                }
            ...
            if (tank.getFluid() != null && tank.getFluidAmount() > 0) {
                TileEntity bottomTile = getWorld().getTileEntity(getPos().down().down());
                if (bottomTile instanceof TileEntityFluidPipe) {
                    if (bottomTile.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, EnumFacing.UP)) {
                        int out = Math.min(40, tank.getFluidAmount());

                        IFluidHandler handler = bottomTile.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, EnumFacing.UP);
    ...
    public CokeOvenRecipe getRecipe() {
        ...
        if (inventory.get(1).isEmpty() || (OreDictionary.itemMatches(inventory.get(1), recipe.output, false) && inventory.get(1).getCount() + recipe.output.getCount() <= getSlotLimit(1) && inventory.get(1).getCount() + recipe.output.getCount() <= recipe.output.getMaxStackSize()))
