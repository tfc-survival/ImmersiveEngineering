...
public class TileEntityAlloySmelter extends TileEntityMultiblockPart<TileEntityAlloySmelter> implements IIEInventory, IActiveState, IGuiTile, IProcessTile {
    ...
    IItemHandler inputHandler = new IEInventoryHandler(4, this, 0, new boolean[]{true, true, false, false}, new boolean[]{false, false, false, true});
    IItemHandler fuelHandler = new IEInventoryHandler(4, this, 0, new boolean[]{false, false, true, false}, new boolean[]{false, false, false, true});
    ...
    public AlloyRecipe getRecipe() {
        ...
        if (recipe == null)
            return null;
        if (inventory.get(3).isEmpty() || (OreDictionary.itemMatches(inventory.get(3), recipe.output, true) && inventory.get(3).getCount() + recipe.output.getCount() <= getSlotLimit(3)) && inventory.get(3).getCount() + recipe.output.getCount() <= recipe.output.getMaxStackSize())
            return recipe;
        return null;
    }
    ...
    public boolean isStackValid(int slot, ItemStack stack) {
        return slot == 0 || slot == 1 || slot == 2 && TileEntityFurnace.isItemFuel(stack);
    }
